...
int main() {
  ...
  printf("Repeatedly enter two words separated by a space (e.g.: cat "
         "someone).\n Stop with: -1 -1\n");
  while (1 == 1) {
    ...
    if (strcmp(first, "-1") == 0 && strcmp(second, "-1") == 0) {
      ...
      int dist = edit_distance(first, second,1); // last argument, 1, indicates that the
                                   // distance table should be printed
